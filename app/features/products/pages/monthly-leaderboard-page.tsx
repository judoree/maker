...
export const meta: Route.MetaFunction = ({ params }) => {
  const date = DateTime.fromObject({
    ...
    .setZone('Asia/Seoul')
    .setLocale('ko');
  return [{ title: `Best of ${date.toLocaleString({ month: 'long', year: '2-digit' })}` }];
};
...
export const loader = ({ params }: Route.LoaderArgs) => {
  ...
  const date = DateTime.fromObject({ year: parsedData.year, month: parsedData.month }).setZone('Asia/Seoul');
  if (!date.isValid) {
...
export default function MonthlyLeaderboardPage({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-10">
      <HeroSection title={`Best of ${urlDate.toLocaleString({ month: 'long', year: '2-digit' })}`} />
